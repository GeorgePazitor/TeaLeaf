...
//calculates global statistics for the simulation field
void field_summary() {
    ...
    //sum local rank totals across the entire network to mpi process 0
    tea_sum(vol);
    tea_sum(mass);
    tea_sum(ie);
    tea_sum(temp);
    ...
    //At the end compares with referece solutions for test problems and reports percentage difference and pass/fail status 
    if (complete) {
        if (parallel.boss && test_problem >= 1) {
            double qa_diff = 0.0;

            //from known analytical/reference solutions
